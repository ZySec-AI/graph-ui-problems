...
function GraphVisualizationInner({ data }: GraphVisualizationProps) {
    ...
    const graphNodes: Node[] = data.nodes.map((node, i) => {
      ...
      return {
        ...
        data: {
          ...node,
          selected:
            selectedGroups.size === 0 || selectedGroups.has(node.group || ""),
          searchMatch:
            searchTerm === "" ||
  ...
  return (
    <div className="w-full h-full">
      ...
      {tooltipNode && (
        <div className="fixed top-[12vh] right-4 z-50 p-3 rounded-lg bg-slate-800/95 backdrop-blur-sm border border-slate-700 shadow-xl text-white max-w-xs">
          <div className="font-semibold mb-2 pb-2 border-b border-slate-700 flex items-center gap-2">
            <div
              className="w-3 h-3 rounded-full"
              style={{ backgroundColor: tooltipNode.style?.color || "#64748b" }}
            />
            {tooltipNode.label}
          </div>

          {tooltipNode.properties &&
            Object.keys(tooltipNode.properties).length > 0 && (
              <div className="grid gap-1 max-h-60 overflow-auto">
                {Object.entries(tooltipNode.properties).map(([key, value]) => (
                  <div
                    key={key}
                    className="grid grid-cols-[1fr,auto] gap-2 text-sm"
                  >
      ...
      {tooltipEdge && (
        <div className="fixed top-[10vh] right-4 z-50 p-3 rounded-lg bg-slate-800/95 backdrop-blur-sm border border-slate-700 shadow-xl text-white max-w-xs">
          <div className="font-semibold mb-2 pb-2 border-b border-slate-700">
            {tooltipEdge.label || "Relationship"}
          </div>
