...
function App() {
  const [graphData, setGraphData] = useState<GraphData | null>(sampleData as unknown as GraphData)
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!json.nodes || !json.edges || !Array.isArray(json.nodes) || !Array.isArray(json.edges)) {
          ...
          throw new Error("Invalid JSON format: Expected 'nodes' and 'edges' as arrays")
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-white">
      <header className="border-b border-slate-700 bg-slate-800/50 backdrop-blur-sm p-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            {graphData?.meta?.title || "Interactive Knowledge Graph Visualization"}
          </p>
        </div>
      ...
      {notification && (
        <div
          className={`px-4 py-2 ${notification.type === "success" ? "bg-emerald-500/20 text-emerald-200" : "bg-red-500/20 text-red-200"} border-l-4 ${notification.type === "success" ? "border-emerald-500" : "border-red-500"}`}
        >
        ...
        {graphData ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center p-8 rounded-xl bg-slate-800/50 backdrop-blur-sm border border-slate-700 max-w-md">
              ...
              <h2 className="text-xl font-semibold mb-2">No Graph Data Loaded</h2>
              <p className="text-slate-400 mb-4">Upload a JSON file with nodes and edges to visualize your graph.</p>
              <button
                onClick={triggerFileUpload}
